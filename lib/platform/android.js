...
var titaniumConfigure = function(options, props, callback) {
	...
	prompt.get([
		{
			message:"Do you have the Android SDK installed already? [(y)es/(n)o/(s)kip]",
			name:"doAndroid"
		}
	],
	function(err, result) {
		if (err) {
			logger.error(err);
			callback(err);
		} else if (/^y/i.test(result.doAndroid)) {
			...
			pc.getPathFromStdin((props.androidSDKPath || ''), function(aPath) {
				try {
					if (fs.statSync(aPath).isDirectory() && 
						fs.existsSync(path.join(aPath, 'platform-tools', 'adb'))
					   ) {
					...
					titaniumConfigure(options, props, callback); 
				}
			});
		} else if (/^n/i.test(result.doAndroid)) {
			logger.info('Download and install the Android SDK from this URL:');
			logger.info('');
			logger.info('    http://developer.android.com/sdk/index.html');
			logger.info('');
			logger.info('Then try running `titanium configure android` again to configure for Android development');
			callback();
		} else if (/^s/i.test(result.doAndroid)) {
			callback();
		} else {
			logger.error('Invalid response, try again.');
			titaniumConfigure(options, props, callback);
		}
	});
};
...
var titaniumBuild = function(options, tiapp, callback) {
	try {
	    ...
	    exec(constants.PYTHON + ' "' + tiBuilderPath + '" build ' + tiapp.name + ' "' + __self.paths.sdk + '" "' + options.path + '" ' + tiapp.id, 
    ...
    async.parallel([ 
        ...
        function(parallelCallback) {
            async.series([
                ...
                function(callback) {
                    if (!doesAvdExist) {
                        avdCreate(avdName, options.avdTargetId, options.avdSkin, function(err) {
                            callback(err);
                        });
                    } else {
                        callback();
...
var avdExists = function(avdName, callback) {
	fs.exists(getAndroidAvdPath(avdName + '.ini'), callback);
};
