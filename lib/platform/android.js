...
//############## MODULE INTERFACE ################//
function Android(sdkPath, titaniumPath, _logger) {
	__self = this;
	logger = _logger;
...
//############## PRIVATE FUNCTIONS ######################//
var titaniumBuild = function(sdkVersion, appName, appPath, appId, callback) {
	try {
		var tiBuilderPath = path.join(__self.paths.titanium, sdkVersion, 'android', 'builder.py');
	    exec('"' + tiBuilderPath + '" build ' + appName + ' "' + __self.paths.sdk + '" "' + appPath + '" ' + appId, 
