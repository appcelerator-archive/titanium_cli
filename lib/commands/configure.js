...
function mobileSDKLocator() {
	...
	fs.readdir(props.sdkRoot, function(err, files) {
		...
		if (files.length > 1) {
			support.debug("Titanium SDK located: " + props.sdkRoot);
			getUserConfig();
		} else {
...
function androidSDKLocator() {
	...
	prompt.get([{
		...
		if (/y/i.test(result.doAndroid)) {
			...
			pc.getPathFromStdin(function(aPath) {
				try {
					if (fs.statSync(aPath).isDirectory()) {
						// TODO: validate in some way that this actually is an android sdk
						support.debug('Android SDK Path: ' + aPath);
						props.androidSDKPath = aPath;
						mobileSDKLocator();
					}
				} catch (e) {
					// TODO: warn, try again or continue without it 
					suppport.error('Android SDK not found at "' + aPath + '"');
					process.exit();
				}
			});
		} else {
...
function iOSSDKLocator() {
	...
	async.waterfall(
		[
			...
			// validate the developer path
			function(developerPath, callback) {
				props.developerPath = validate.pathDoesExist(developerPath, function(val) {
					callback('Could not find XCode Developer folder. You will not be able to build iOS apps');
				});
				support.debug('Found Developer path: ' + developerPath);

				var iosSdkPath = path.join(props.developerPath, constants.IOS_SDK_PATH);
				validate.pathDoesExist(iosSdkPath, function(val) {
					callback('Could not find iOS SDK folder. You will not be able to build iOS apps');
				});
				support.debug('Found iOS SDK path: ' + iosSdkPath);		
