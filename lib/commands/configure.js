...
function iOSSDKLocator() {
	...
	async.waterfall(
		[
			...
			function(developerPath, callback) {
				if (!path.existsSync(developerPath)) {
					callback('Could not find XCode Developer folder at "' + developerPath + '"');
					return;
				}
				...
				if (!path.existsSync(iosSdkPath)) {
					callback('Could not find iOS SDK folder at "' + iosSdkPath + '"');
					return;
				}
		...
		function(err, result) {
			if (err) { 
				logger.warn(err); 
				logger.warn('You will not be able to build iOS apps');
			}
			androidSDKLocator(); 
		}
	);
