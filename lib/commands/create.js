...
exports.execute = function(args, options, logger) {
	...
	for (var i = 0; i < options.target.length; i++) {
		if (!isValidTarget(options.target[i])) {
			logger.error('Invalid target "' + options.target[i] + '". Must be one of the following: [' + getValidTargets() + ']');
			return;
		}
	}
...
var isValidTarget = function(target) {
	var targets = constants.CREATE_TARGETS
...
var getValidTargets = function() {
	var targets = constants.CREATE_TARGETS;
	var string = '';
	for (var i = 0; i <targets.length; i++) {
		string += targets[i] + (i+1 == targets.length ? '' : ',');
	}
	return string;
};
