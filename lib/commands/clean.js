...
exports.execute = function(args, options, _logger) {
	...
	if (!fs.existsSync(rootPath)) { logger.die('"' + rootPath + '" is not a project directory'); }
	if (!fs.existsSync(tiappPath)) { logger.die('"' + rootPath + '" is not a Titanium project directory (no tiapp.xml found)'); }
