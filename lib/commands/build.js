...
var buildAndroid = function(tiapp, callback) {
	try {
		var android = new Android(userConf.androidSDKPath, userConf.mobileSdkRoot, logger);
...
var buildIphone = function(tiapp, callback) {
	try {
...
var buildMobileweb = function(tiapp, callback) {
	try {
		var Mobileweb = require('../platform/mobileweb');
		var mw = new Mobileweb(userConf.mobileSdkRoot, logger);
		
		logger.debug('Starting Mobileweb build');
		mw.titanium.build({ sdk: options.sdk, path: options.path }, function(err) {
			if (err) {
				logger.error('Mobileweb build failed');
				logger.error(err);
			} else {
				logger.debug('Mobileweb build completely successfully');
			}
			callback();
		});
	} catch (e) {
		...
		config.getTiappXml(path.join(options.path,'tiapp.xml'), function(tiapp) {
			...
			targets.forEach(function(target) {
				switch (target) {
					...
					case 'mobileweb':
						functions.push(function(callback) {
							buildMobileweb(tiapp, function(err) {
								if (err) { logger.error(err); }
								callback();
							});
						});
					...
					case 'iphone':
						functions.push(function(callback) {
							buildIphone(tiapp, function(err) {
								if (err) { logger.error(err); }
								callback();
							});
						});
