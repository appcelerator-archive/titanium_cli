...
var deployMobileweb = function(tiapp, callback) {
	try {
		var Mobileweb = require('../platform/mobileweb');
		var mw = new Mobileweb(userConf.mobileSdkRoot, logger);

		logger.debug('Starting Mobileweb deploy');
		mw.titanium.deploy(options, function(err) {
			if (err) {
				logger.error('Failed to deploy Mobileweb app ' + tiapp.name);
				logger.error(err);
			} else {
				logger.debug('Mobileweb app ' + tiapp.name + ' has been deployed');
			}
			callback();
		});
	} catch (e) {
