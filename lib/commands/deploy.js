...
var deployAndroid = function(tiapp, deployCallback) {
	...
	logger.info('Deploying ' + tiapp.name + ' to Android device...');
	async.parallel([
		// build apk
...
var deployMobileweb = function(tiapp, deployCallback) {
	var builderPath = '"' + path.join(userConf.mobileSdkRoot, options.sdk, 'mobileweb', 'builder.py') + '"';
	var buildPath = '"' + path.join(options.path, 'build', 'mobileweb') + '"';
	logger.debug('Starting mobileweb build for ' + tiapp.name);
	...
	if (!options.webroot) {
		...
		deployCallback('Webroot "' + options.webroot + '" does not exist');
		return;
	}
	...
	async.series([
		function(callback) {
			if (options.nobuild) {
				...
				exec(builderPath + ' "' + options.path + '" run', function(err, stdout, stderr) {
		    		if (err) { 
		    			logger.error('Failed to build mobileweb project for ' + tiapp.name);
		    			logger.error(err); 
		    		} else {
		    			logger.debug('Mobileweb project has been built for ' + tiapp.name);
		    		}
		    		callback(err);
		    	});
			}
		...
		function(callback) {
			try {
	    		...
	    		exec('cp -rf ' + path.join(buildPath, '*') + ' ' + options.webroot, function(err, stdout, stderr) {
	    			if (!err) { logger.debug('Mobileweb app ' + tiapp.name + ' deployed to ' + options.webroot); }
	    			callback(err);
	    		});
	    	} catch (e) {
		...
		config.getTiappXml(path.join(options.path,'tiapp.xml'), function(tiapp) {
			...
			targets.forEach(function(target) {
				switch (target) {
					...
					case 'mobileweb':
						functions.push(function(callback) {
							deployMobileweb(tiapp, function(err) {
								if (err) { logger.error(err); }
								callback();
							});
						});
