...
function runAndroid(tiapp, options) {
    var builderPath = path.join(userConf.mobileSdkRoot, options.sdk, 'android', 'builder.py');
    var apkPath = path.join(options.path, 'build', 'android', 'bin', 'app.apk');
    var android = new Android(userConf.androidSDKPath);

    // Establish avd name, or create one from a target id and skin
    // TODO: Validate targets and skins
    options.avdTargetId = options.avdTargetId || 1;
    options.avdSkin = options.avdSkin || 'HVGA';
    var avdName = options.avdName || 'titaniumcli_' + options.avdTargetId + '_' + options.avdSkin;
    var serial;
    var avdExists = false;
    ...
    async.parallel([
        ...
        function(parallelCallback) {
            ...
            exec('"' + builderPath + '" build ' + tiapp.name + ' "' + userConf.androidSDKPath + '" "' + options.path + '" ' + tiapp.id, 
        ...
        if (err) {
            ...
            async.series([
                ...
                function(callback) {
                    ...
                    android.adb.runApp(serial, tiapp.id, '.TonytestActivity', function(err) {
                        callback(err);
                    });
                }
            ],
