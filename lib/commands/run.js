...
exports.doc = {
    ...
    usage: 'titanium run [iphone,ipad,android,mobileweb,blackberry] [OPTIONS]',
    options: [
...
exports.execute = function(args, _options, _logger, _cmdCallback) {
    ...
    } else if (!fs.existsSync(path.join(options.path,'tiapp.xml'))) {
    ...
    config.getTiappXml(path.join(options.path,'tiapp.xml'), function(tiapp) {
        ...
        //once we have a valid list of targets, just launch the first valid target (from tiapp.xml) if none was specified
        if (!args[0]) {
            targets = [targets[0]];
        }
