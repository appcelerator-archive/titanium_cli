...
function runAndroidEmulator(android_version) {
    ...
    tiSDK.runScript("'"+path.join(conf.mobileSdkRoot, ti_SDK, 'android', 'builder.py')+"' simulator '"+conf.tiapp.name+"' '"+conf.androidSDK+"' '"+conf.projectRoot+"' '"+conf.tiapp.id+"' "+android_version+" && "+conf.androidSDK+"platform-tools/adb logcat | grep Ti");
}
