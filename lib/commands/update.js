...
exports.doc = {
	...
	options: [
		...
		['-u, --url <url>', 'Update Titanium SDK from a URL']
	],
	needsConfig: true
};
			...
			function(parallelCallback) {
				async.waterfall([
					...
					function(json, callback) {
						if (options.url) {
							var pieces = options.url.match(/^http[s]*:\/\/([^\/]+)\/(.*)$/);
							callback(null, {
								host: pieces[1],
								port: 80,
								path: '/' + pieces[2],
								filename: path.join(sdkRoot, 'titanium-url-download.zip'),
								version: 'from ' + options.url
							});
							return;
						}
					...
					function(sdkData, callback) {
						try {
							if (!options.url) {
								...
								if (fs.existsSync(sdkData.filename) && !options.force) {
							...
							var downloadFile = function(res) {
								...
								res.on('data', function(chunk) {
									...
									process.stdout.write(RESTART_LINE_CHAR + '[INFO]  Download Progress: ' + Math.floor(progress / fileSize * 100) + '%');
								}).on('end', function() {
									downloadFile.end();
									if (res.statusCode != 200) {
										logger.debug(res);
										logger.error('Download failed with code ' + res.statusCode);
									} else {
										console.log(' ');
